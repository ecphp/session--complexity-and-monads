...
\begin{frame}
  ...
  \note[item]{
    ...
    smaller and formal definition worth ten thousands words... or not.
  }

  \note[item]{
    Let f a transformation, a function which takes an input of type A and return
    a type B.
  }
  \note[item]{
...
\begin{frame}
  ...
  \note[item]{
    It exists at least one x in type A such that it does not exists an image
    of x through f in type B.
  }
\end{frame}
...
\begin{frame}
  ...
  \note[item]{
    A very basic example of total, partial and pure function now to make sure
    that the definitions are well understood.
  }
  \note[item]{
...
\begin{frame}
  ...
  \note[item]{
    Let's move on to some contrete code and see how these definitions and rules
    applies.
  }
...
\begin{frame}
  ...
  \note[item]{
    The first option we have is to return the controversial null value.
  }
  \note[item]{
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item This value wrapper is actually the ``Maybe'' monad,
...
\begin{frame}[plain]
  ...
  \note[item]{
    ...
    the concept, nothing else, nothing more for now.
  }
  \note[item]{
    Be careful...
  }
...
\begin{frame}
  ...
  \note[item]{
    ...
    Promises, in PHP with... Argh there is no core Monad implementation !
    There is also no ``official'' monad package or any standard interface for
    them.
  }
\end{frame}
